...
export interface BetProgress {
  ...
  period_end?: string;
  won?: boolean;
  // Present on an XRP loss — the device must send this on-chain forfeit itself.
  xrp_forfeit?: { amount: number; dest: string };
  error?: string;
}
...
class SelfBetService {
  ...
  }): Promise<{ ok: boolean; bet_id?: string; error?: string }> {
